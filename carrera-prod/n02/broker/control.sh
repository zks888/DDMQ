...
function start() {
    ...
    cd ${workspace}
    nohup sh bin/mqbroker -c ${workspace}/conf/2m-2s-async/broker-b.properties >> ${LOG_HOME}/broker-b.log 2>&1 &
    echo "running: nohup sh bin/mqbroker -c ${workspace}/conf/2m-2s-async/broker-b.properties >> ${LOG_HOME}/broker-b.log 2>&1 &"

    nohup sh bin/mqbroker -c ${workspace}/conf/2m-2s-async/broker-a-s.properties >> ${LOG_HOME}/broker-a-s.log 2>&1 &
    echo "running: nohup sh bin/mqbroker -c ${workspace}/conf/2m-2s-async/broker-a-s.properties >> ${LOG_HOME}/broker-a-s.log 2>&1 &"
